          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-3 py-2 text-sm font-medium text-gray-600 hover:text-emerald-700 rounded-md transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <nav className="px-4 py-3 space-y-1">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className="block px-3 py-2 text-base font-medium text-gray-600 hover:text-emerald-700 rounded-md"
              >
                {link.label}
              </Link>
            ))}
            {phone && (
              <a
                href={`tel:${phone.replace(/\D/g, "")}`}
                className="flex items-center gap-2 px-3 py-2 text-base font-medium text-emerald-700"
              >
                <Phone className="w-4 h-4" />
                {phone}
              </a>
